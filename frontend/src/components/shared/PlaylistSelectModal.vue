...
const props = defineProps({
  ...
  trackIds: { type: Array, required: true, default: () => [] } // 추가할 곡들의 ID 배열
})
...
// 💡 모달이 열릴 때 플레이리스트 목록이 비어있다면 불러옵니다.
onMounted(async () => {
...
// 💡 수동 플레이리스트('list' 타입)만 필터링하고 검색어를 적용합니다.
...
const handleSelect = async (playlistId) => {
  ...
  try {
    // 💡 스토어에 이미 작성된 addTracksToPlaylist 메서드 활용!
