...
import { MoreVertical, Disc, Users, GripVertical } from 'lucide-vue-next'
...
const virtualTracks = computed(() => props.localTracks ?? [])

const { list, containerProps, wrapperProps } = useVirtualList(virtualTracks, {
  itemHeight: 64,
  overscan: 8,
})
</script>
...
<template>
  <div v-if="playlistId" class="hidden md:block pb-2">
    <Table class="border-b">
      ...
      >
        ...
        >
          <TableCell class="w-8 p-0 text-center align-middle" @click.stop>
  ...
  <div v-else v-bind="containerProps" class="hidden md:block pb-2 max-h-[min(70vh,calc(100dvh-14rem))] overflow-y-auto">
    <Table class="border-b">
