...
| **0.8.0-alpha.5** | 선택 곡 일괄 태그, 체크박스 셀 전체 토글, YouTube 임포트 한도·429 안내 | — |
| **0.8.0-alpha.6** | 스마트 믹스 조건 select 먹통 수정 (포털 팝오버 z-index) | — |
...
## 0.8.0-alpha.5 — 일괄 태그·선택 UX·임포트 한도 ✅

| # | 작업 | 상태 |
|---|------|------|
| 0.8.0-alpha.5.1 | `PATCH /api/tracks/tags/bulk` — 곡별 합집합·차집합 병합, 3단 상태 다이얼로그 | ✅ |
| 0.8.0-alpha.5.2 | 앨범·아티스트·재생목록 상세 트랙에 `tags` 배열 동봉 | ✅ |
| 0.8.0-alpha.5.3 | 체크박스 셀 전체를 클릭 영역으로 (label 위임, 헤더 전체선택 포함) | ✅ |
| 0.8.0-alpha.5.4 | YouTube 임포트 시간당 5회 → 30회 | ✅ |
| 0.8.0-alpha.5.5 | 429 응답에 `Retry-After` 기반 재시도 안내 | ✅ |

임포트가 4~5회 뒤 막히던 것은 yt-dlp가 아니라 자체 rate limit이었다. 실패한 요청도
할당량을 먹는데다 429 본문이 `Too Many Requests`뿐이라 원인 파악이 불가능했다.

---

## 0.8.0-alpha.6 — 포털 팝오버 z-index ✅

| # | 작업 | 상태 |
|---|------|------|
| 0.8.0-alpha.6.1 | `--z-dropdown` 50 → 110, `--z-dialog: 100` 토큰 추가 | ✅ |
| 0.8.0-alpha.6.2 | `SelectContent`·`PopoverContent`를 하드코딩 `z-50`에서 토큰으로 이전 | ✅ |

스마트 믹스 조건 select가 rating·>=에서 고정되던 원인. body로 포털되는 메뉴가 z-50이라
z-[100] 다이얼로그 오버레이 뒤에 깔려 클릭이 오버레이에 먹혔다. 바인딩 문제가 아니었다.
